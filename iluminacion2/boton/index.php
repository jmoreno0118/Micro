 ...
 {	
   ...
   try   
    {$sql='';
	 ...
	 $where=' WHERE estudiostbl.nombre="iluminacion" and usuariostbl.usuario=:usuario';
...
if((isset($_POST['accion']) and $_POST['accion']=='Ver OT'))
 {
  ...
  try
  {
    $sql='SELECT nombre FROM estudiostbl WHERE ordenidfk=:id';
      $s=$pdo->prepare($sql);
      $s->bindValue(':id',$_POST['id']);
      $s->execute();
	}
...
/**************************************************************************************************/
  ...
  if(isset($_POST['accion']) and ($_POST['accion']=='verci' or $_POST['accion']=='volverci'))
  {
    include $_SERVER['DOCUMENT_ROOT'].'/reportes/includes/conectadb.inc.php';
    if ($_POST['accion']=='volverci')
	{
     verRecs($_POST['idot']);
	}
   else 
...
//  if(isset($_GET['volverpts']))
  if (isset($_POST['accion']) and $_POST['accion']=='volverpts')
  {
   include $_SERVER['DOCUMENT_ROOT'].'/reportes/includes/conectadb.inc.php';
...
/**************************************************************************************************/
  if(isset($_POST['accion']) and $_POST['accion']=='capturarci')
  {
   include $_SERVER['DOCUMENT_ROOT'].'/reportes/includes/conectadb.inc.php';
...
******************** */
   try
   {	 //ordenidfk=:idot, lo extraje del $sql de abajo
    $sql='UPDATE recsilumtbl SET
          fecha=:fecha,
		  largo=:largo,
		  ancho=:ancho,
   ...
   try
   {
    ...
    try
    {
     $sql="SELECT deptostbl.id FROM deptostbl INNER JOIN deptorecilumtbl ON deptostbl.id = deptorecilumtbl.deptoidfk WHERE deptorecilumtbl.recilumidfk = :id";
	 $s=$pdo->prepare($sql);
	 $s->bindValue(':id',$_POST['id']);
	 $s->execute();
	 $resultado=$s->fetch();
...
/**************************************************************************************************/
  include 'formabuscaordenesilum.html.php';
  exit();
...
/**************************************************************************************************/
  function verPuntos($id = ""){  
   global $pdo;
   // esta busqueda la modifique para traer el numero de ot
   try
   {
