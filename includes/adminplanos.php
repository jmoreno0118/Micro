...
if (isset($_POST['accion']) and $_POST['accion']=='borraplano')
{
 ...
 $arch=$_SERVER['DOCUMENT_ROOT'].str_replace('?','',$_SERVER['REQUEST_URI']).$nombre['nombrearchivado'];
    if (file_exists($arch)) {
	try{	
...
/*	$archivotipo=exif_imagetype($_FILES['archivo']['tmp_name']);
	...
	$_SERVER['DOCUMENT_ROOT'].str_replace('?','',$_SERVER['REQUEST_URI']).$nombrearchivar);
	if (!$semovio){
...
//	  include $_SERVER['DOCUMENT_ROOT'].'/reportes/includes/error.html.php';
	...
	chmod ($_SERVER['DOCUMENT_ROOT'].str_replace('?','',$_SERVER['REQUEST_URI']).$nombrearchivar,0644);
 ...
 $estudio=$_SESSION['quien'];
  try
  {
    $sql='SELECT id, nombre, nombrearchivado, descripcion FROM planostbl
		WHERE ordenidfk=:idot and estudio=:estudio';
	$s=$pdo->prepare($sql);
	$s->bindValue(':idot',$idot);
	$s->bindValue(':estudio',$estudio);
	$s->execute();
