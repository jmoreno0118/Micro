...
/*    // solicita la confirmación para borrar un cliente
  ...
  try
  {
    $resultado=$pdo->query('SELECT ordenestbl.id, ot, razonsocial, planta, municipio, estado 
	FROM ordenestbl
	 ...
	 INNER JOIN usuarioreptbl ON representantetbl.id=usuarioreptbl.representanteidfk
	 INNER JOIN usuariostbl ON usuariostbl.id=usuarioidfk
	 WHERE fechafin IS NULL and nombre="iluminacion" and usuario=$usuarioactal');
  }
  catch (PDOException $e)
  {
